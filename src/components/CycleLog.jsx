...
export default function CycleLog({ user }) {
  ...
  // Notify if user is on period
  useEffect(() => {
    if (!cycles.length) return;
    const today = new Date();
    const onPeriod = cycles.some((c) => {
      const start = new Date(c.start_date);
      const end = c.end_date ? new Date(c.end_date) : null;
      return today >= start && (end === null || today <= end);
    });
    if (onPeriod) {
      // Use Notification API if available, else fallback to alert
      if (window.Notification && Notification.permission === "granted") {
        new Notification("Period Tracker", {
          body: "You are currently on your period. Take care!",
        });
        ...
        if (!window._periodNotified) {
          alert("You are currently on your period. Take care!");
          window._periodNotified = true;
        }
      }
    }
  }, [cycles]);
  ...
  const reload = async () => {
    try {
      ...
      setCycles(res.data || []);
      // Log cycles and their IDs for debugging
      console.log("[DEBUG] Cycles:", res.data);
      // Only dispatch refresh-insights after cycles are set (ensures backend is up-to-date)
      setTimeout(() => {
  ...
  return (
    <Box sx={{ mt: 4 }}>
            ...
            >
              <CardContent sx={{ flexGrow: 1 }}>
                <Typography variant="h6" sx={{ fontWeight: "bold", mb: 1 }}>
                  {c.start_date} to {c.end_date || "ongoing"}
                </Typography>
      ...
      {/* Show cycle details in a modal/panel if loaded (like MedicationReminder) */}
      {cycleDetails && (
        <Paper sx={{ p: 4, mt: 4 }}>
          <Typography variant="h5" sx={{ fontWeight: "bold" }}>
            Cycle Details
          ...
          <Typography sx={{ mt: 2 }}>
            <strong>Start Date:</strong> {cycleDetails.start_date}
            <br />
            <strong>End Date:</strong> {cycleDetails.end_date || "Ongoing"}
            <br />
            <strong>Flow Intensity:</strong> {cycleDetails.flow_intensity}
            <br />
            <strong>Notes:</strong> {cycleDetails.notes}
          </Typography>
          <Button
            sx={{ mt: 2 }}
            variant="outlined"
            color="secondary"
            onClick={() => setCycleDetails(null)}
          >
            Close
          </Button>
        </Paper>
